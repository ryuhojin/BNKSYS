...
<div>
	<br />
	<table class="list_table" style="width: 85%; margin: auto; text-align: center;" border="1">
		<tr>
			<th>번호</th>
			<th>제목</th>
			<th>등록일</th>
		...
		<c:forEach items="${notice}" var="m" varStatus="vs">
			<tr>
				<td onclick="noticedetail('${m.num}')">${m.num }</td>
				<td onclick="noticedetail('${m.num}')">${m.title }</td>
				<td onclick="noticedetail('${m.num}')">${m.date }</td>
				<td style="padding-left: 5px;">
					<span>
						<button type="button" id="_btnDelete" onclick="deleteNotice('${m.title}', '${m.author }', '${sessionScope.login.id }')">삭제</button>
					</span>
				</td>
			</tr>
		</c:forEach>
	...
	<br /> <br />
	<input type="button" value="추가 " class="button" style="width: 50px; margin-left: 50%;" onclick="location.href='./foodcontrol?action=noticeaddform'">
</div>
...
<script type="text/javascript">
	...
	function deleteNotice(_title, _author, _loginid) {
		location.href = './foodcontrol?action=noticedelete&title=' + _title
				+ '&author=' + _author + '&loginid=' + _loginid;
	}
</script>
</body>
</html>